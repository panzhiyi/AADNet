...
def voxelize(coord, mask, voxel_size=0.05, hash_type='fnv', mode=0):
...
def crop_pc(coord, feat, label, split='train',
            ...
            downsample=True, variable=True, shuffle=True, mask=None):
    if voxel_size and downsample:
        # Is this shifting a must? I borrow it from Stratified Transformer and Point Transformer.
        coord -= coord.min(0)
        uniq_idx = voxelize(coord, mask, voxel_size)
        # if mask is None:
        #    uniq_idx = voxelize(coord, mask, voxel_size)
        # else:
        #    uniq_idx = voxelize_sparse(coord, mask, voxel_size)
