...
class S3DIS(Dataset):
    ...
    def __init__(self,
        ...
        raw_root = os.path.join(data_root, data_raw)
        print(f"{raw_root} load successfully")
        print(f"label rate is {str(10**(2-rate))}%")
        self.raw_root = raw_root
        data_list = sorted(os.listdir(raw_root))
        data_list = [item[:-4] for item in data_list if 'Area_' in item]
        if split == 'train' or split == 'finetune':
            self.data_list = [
        ...
        if presample and not os.path.exists(filename):
            ...
            for item in tqdm(self.data_list, desc=f'Loading S3DISFull {split} split on Test Area {test_area}'):
                ...
                if voxel_size:
                    coord, feat, label, mask = cdata[:,0:3], cdata[:, 3:6], cdata[:, 6:7], cdata[:, 6 + self.rate]
                    if self.rate != 0 and self.split == 'train':
                        label[mask == 0] = self.ignore_index
                    else:
                        mask = None
                    uniq_idx = voxelize(coord, voxel_size, mask)
    ...
    def __getitem__(self, idx):
        ...
        if self.presample:
            ...
            cdata = np.load(data_path).astype(np.float32)
            cdata[:, :3] -= np.min(cdata[:, :3], 0)
            coord, feat, label, mask = cdata[:, :3], cdata[:, 3:6], cdata[:, 6:7], cdata[:, 6 + self.rate]
            if self.rate != 0 and self.split == 'train':
                label[mask == 0] = self.ignore_index 
            else:
                mask = None
            coord, feat, label = crop_pc(
                ...
                downsample=not self.presample, variable=self.variable, shuffle=self.shuffle, mask=mask)
